...
func initShuttleYML(app, domain, host, user string, port int, email, servicePort, strategy string) string {
	...
	b.WriteString("  # Uncomment when SSH is only reachable through WireGuard.\n")
	b.WriteString("  # vpn:\n")
	b.WriteString("  #   required: true\n")
	b.WriteString("  #   interface: wg0\n")
	fmt.Fprintf(&b, "  #   check_host: %s\n", host)
	fmt.Fprintf(&b, "  #   check_port: %d\n", port)
	b.WriteString("\n")
	b.WriteString("deploy:\n")
	fmt.Fprintf(&b, "  strategy: %s\n", strategy)
	b.WriteString("  timeout: 120\n")
	b.WriteString("  # Promotion budget and optional public zero-downtime proof.\n")
	b.WriteString("  promotion_slo_seconds: 30\n")
	b.WriteString("  total_slo_seconds: 30\n")
	fmt.Fprintf(&b, "  # availability_url: https://%s/health\n", domain)
	b.WriteString("  # availability_interval_ms: 250\n")
