...
function makeDefaultConfig(): ShuttleConfig {
	return {
		app: 'myapp',
...
describe('DevManager', () => {
	...
	beforeEach(() => {
		devDir = mkdtempSync(path.join(os.tmpdir(), 'deploy-shuttle-dev-test-'))
		manager = new DevManager(devDir)
	...
	describe('up() — Caddyfile generation', () => {
		it('writes Caddyfile.dev with the config domain', async () => {
			await manager.up(makeDefaultConfig())

			const content = readFileSync(path.join(devDir, '.shuttle', 'Caddyfile.dev'), 'utf8')
			expect(content).toContain('myapp.example.com')
		})

		it('uses config.dev.domain when set', async () => {
			await manager.up({ ...makeDefaultConfig(), dev: { domain: 'myapp.local' } } as ShuttleConfig)
	...
	describe('up() — compose file generation (no SSL)', () => {
		it('writes docker-compose.dev.yml', async () => {
			...
			const content = readFileSync(path.join(devDir, 'docker-compose.dev.yml'), 'utf8')
	...
	describe('up() — docker compose invocation', () => {
		...
		it('throws when docker compose exits non-zero', async () => {
			...
			await expect(manager.up(makeDefaultConfig())).rejects.toThrow(
