...
export class DockerHubRegistry implements RegistryProvider {
	...
	async resolve(config: ShuttleConfig, tag: string): Promise<string> {
		...
		const url = config.registry?.url ?? 'docker.io'
		return `${url}/${tag}`
	}
	...
	async distribute(host: string, imageRef: string, config: ShuttleConfig): Promise<void> {
		...
		if (username && passwordEnv) {
			...
			if (!password) {
				throw new Error(
					`Environment variable ${passwordEnv} is not set (required for Docker Hub login)`,
				)
			}
			const url = config.registry?.url ?? 'docker.io'
...
export function createDockerHubRegistry(docker: DockerManager, ssh: SSHManager): RegistryProvider {
