...
export class SecretsManager {
	 ...
	 * remote host at /opt/shuttle/<project>/<app>/.env with mode 0600.
	 */
	async push(host: string, app: string, project?: string): Promise<void> {
		const secrets = await this.loadAll()
		...
		const projectDir = project ?? app
		const remotePath = `/opt/shuttle/${projectDir}/${app}/.env`

		// Ensure the directory exists on the remote
		await this.ssh.exec(host, `mkdir -p ${shellEscape(`/opt/shuttle/${projectDir}/${app}`)}`)

		await this.ssh.uploadContent(host, envContent, remotePath, 0o600)
...
export const secrets = new SecretsManager(defaultSsh)
