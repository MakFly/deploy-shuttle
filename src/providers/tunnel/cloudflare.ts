...
export class CloudflareTunnelProvider implements TunnelProvider {
	...
	async start(host: string, config: ShuttleConfig): Promise<void> {
		...
		const { code, stderr } = await this.ssh.exec(host, [
			'docker run -d',
			`--name ${CONTAINER_NAME}`,
		...
		if (code !== 0) {
			throw new DeployError(`Failed to start Cloudflare Tunnel on ${host}: ${stderr.trim()}`, 'tunnel')
		}
	...
	async getStatus(host: string): Promise<string> {
		const { stdout, code } = await this.ssh.exec(host, `docker inspect ${CONTAINER_NAME} --format '{{.State.Status}}' 2>/dev/null`)
