...
export class SlackNotificationsProvider implements NotificationsProvider {
	...
	async notify(config: ShuttleConfig, event: ShuttleEvent, payload: Record<string, unknown>): Promise<void> {
		...
		const blocks: Record<string, unknown>[] = [
			{ type: 'section', text: { type: 'mrkdwn', text: `${emoji} *${config.app}* — \`${event}\`` } },
		]
		if (fields.length > 0) {
			blocks.push({ type: 'section', fields })
		}
		blocks.push({ type: 'context', elements: [{ type: 'mrkdwn', text: `Shuttle • ${new Date().toISOString()}` }] })
