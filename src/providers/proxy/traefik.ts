...
export class TraefikProxyProvider implements ProxyProvider {
	...
	async apply(host: string, config: ShuttleConfig, upstreams: Map<string, string>): Promise<void> {
		...
		await this.ssh.uploadContent(host, dynamicConfig, `${TRAEFIK_DYNAMIC_DIR}/${config.app}.yml`, 0o644)
		await this.ensureTraefikRunning(host)
	}

	async switchUpstream(host: string, config: ShuttleConfig, domains: string[], newPort: number): Promise<void> {
	...
	async getStatus(host: string): Promise<string> {
		const { stdout, code } = await this.ssh.exec(host, `docker inspect ${TRAEFIK_CONTAINER_NAME} --format '{{.State.Status}}' 2>/dev/null`)
...
`
	...
	private generateDynamicConfig(config: ShuttleConfig, upstreams: Map<string, string>): string {
		...
		const servers = Array.from(upstreams.values()).map((url) => `          - url: "${url}"`).join('\n')
...
`
	...
	private async ensureTraefikRunning(host: string): Promise<void> {
		const { code } = await this.ssh.exec(host, `docker inspect ${TRAEFIK_CONTAINER_NAME} --format '{{.State.Status}}' 2>/dev/null`)
		if (code === 0) {
			logger.debug('Traefik container already running')
			return
		}
		logger.info('Starting Traefik container...')
		const { code: runCode, stderr } = await this.ssh.exec(host, [
			'docker run -d',
