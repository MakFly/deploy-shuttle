...
- **GitHub community perk**: buyers who fill the optional "GitHub username" Payment Link custom field are invited to the private Pro repo (`GITHUB_PRO_REPO`, refund = removal). Perk only — the license key stays the entitlement; GitHub failures never fail the webhook (`license-server/src/lib/github.ts`).
